...
import { useManualGroupFileUpload } from "@/components/discovery/useManualGroupFileUpload";
...
export function MasterGroupFileSection({
  ...
  } = useSessionAction(sessionId, "/group-file/confirm");
  ...
  const [groupFileCandidateChoice, setGroupFileCandidateChoice] = useState("");

  const {
    pending: selectingGroupFile,
    error: groupFileSelectError,
    run: runSelectGroupFileCandidate,
  } = useSessionAction(sessionId, "/group-file/select");
  ...
  const groupFileBusy =
    manualGroupFileUploading || confirmingGroupFile || selectingGroupFile;
  ...
  // it manually.
  const [forceShowGroupFileCandidates, setForceShowGroupFileCandidates] =
    useState(false);

  const handleConfirmGroupFile = async () => {
    if (!sessionId) return;

    const result = await runConfirmGroupFile();
  ...
  const handleSelectGroupFileCandidate = async () => {
    if (!groupFileCandidateChoice || !sessionId) return;

    const result = await runSelectGroupFileCandidate({
      group_file_name: groupFileCandidateChoice,
    });
    ...
    <div className="rounded border-2 border-yellow-500 bg-yellow-950/40 p-4">
      ...
      {discovery.group_files_found === 0 &&
        ...
        !netNewAcknowledged && (
          <div className="mb-3 flex items-start gap-2 text-yellow-300">
            ...
            <span>
              No master group file found — every discovered group will be
              treated as net-new. Expected for a net-new client with
              nothing in QMS yet to cross-reference against.
            </span>
          </div>
        )}

      <input
        ref={manualGroupFileInputRef}
        type="file"
        className="hidden"
        onChange={handleManualGroupFileChange}
      />
      ...
      (!discovery.selected_group_file_name ||
        ...
        <GroupFileCandidatePicker
          discovery={discovery}
          choice={groupFileCandidateChoice}
          onChoiceChange={setGroupFileCandidateChoice}
          onSelect={handleSelectGroupFileCandidate}
          onCancel={() => setForceShowGroupFileCandidates(false)}
          selecting={selectingGroupFile}
          busy={groupFileBusy}
          error={groupFileSelectError}
        />
      ) : discovery.selected_group_file_name ? (
        <GroupFileSummary
          discovery={discovery}
          busy={groupFileBusy}
          confirming={confirmingGroupFile}
          uploading={manualGroupFileUploading}
          onConfirm={handleConfirmGroupFile}
          onChooseFromDiscovered={() => setForceShowGroupFileCandidates(true)}
          onSelectDifferentFile={openManualGroupFilePicker}
        />
      ) : netNewAcknowledged ? (
        <div className="mt-3 text-green-400">
          ✅ Confirmed net-new client — every group will be treated as
          net-new.
        </div>
      ) : (
        <div className="mt-4 flex gap-3">
          ...
          <button
            onClick={openManualGroupFilePicker}
            disabled={manualGroupFileUploading}
            className="rounded bg-slate-700 px-4 py-2 hover:bg-slate-600 disabled:opacity-50"
          >
            {manualGroupFileUploading ? "Uploading..." : "Select File"}
          </button>
        </div>
      )}

      {(manualGroupFileError || confirmGroupFileError) && (
